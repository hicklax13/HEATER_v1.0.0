...
**Bubba** is HEATER's AI assistant — a folksy, razor-sharp baseball-insider persona (named for Bubba Crosby) reachable from a button on **every page**, in a pop-up chat that **looks, operates, and feels as polished as the claude.ai main chat**. Bubba has **full access to ANY and ALL of the app's data** — historical, live, and future — to read, analyze, and recommend. **HEATER only reads and analyzes; it never makes league moves** — Bubba recommends a lineup or an add/drop and explains the why, but the user executes every transaction manually in their league provider's app. It is the single conversational surface over the entire HEATER brain.
...
### Full data ACCESS — read + analyze only (the load-bearing requirement)
**HEATER is a read-and-analyze product — it does NOT make moves in your fantasy league** (owner product boundary, 2026-06-20). Bubba reaches **any and all** historical / live / future data to read, analyze, and **recommend** — but it never writes to the league. Every transaction is executed **manually by the user** in their league provider's app (Yahoo, etc.).
...
- **RECOMMEND, never execute:** Bubba can propose a lineup, an add/drop, a trade, or a streamer — with the reasoning and the projected impact — rendered as a clear card. It has **no league-write / action tools**; the user makes the actual move themselves in the provider's app. ("Full access and control of ANY/ALL data" = unrestricted querying + analysis of the data, NOT control of the league.)
- This gives Bubba the whole brain to reason over while keeping HEATER strictly read-only on the league.
...
- **Phase B4 — Recommendation depth + future-data extensibility:** richer structured recommendations (proposed lineups / add-drops / trades with projected impact, rendered as cards the user acts on **manually** in their provider's app — Bubba never executes), and the pattern for auto-exposing future data/features to Bubba (schema-card + tool-wrapper convention). No league-write tools.

## Error handling
Backend never 500s on chat (graceful error in the response, like today's `st.warning` paths → HTTP body). Provider/key errors return a clear message. Over-cap (managed, free taste exhausted) → a structured "upgrade" response the UI renders as a prompt. (No league-write tools exist, so there is no mutation path to fail.)

## Testing
- Backend: DB-free service tests with a fake provider (monkeypatch `providers.chat`) + in-memory stores; the identity-mapping + per-tier cap logic; a guard that the tool surface stays **read-only** (no league-write tool ever registered). Reuse the M2 fake-service pattern.
...
- **Read-only boundary (owner product decision)** — Bubba never writes to the league; it recommends, the user executes manually in the provider's app. No action/write tools are registered; a test guards the tool surface stays read-only.
