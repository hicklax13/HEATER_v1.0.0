...
- **Task 1 root cause (confirmed / observed):**
  - (a) name mismatch — CONFIRMED. Scratch repro (`scratchpad/repro_finder.py`) output: `raw .get(): []` for `{"🏆 Team Hickey": [1,2,3], ...}.get("Team Hickey", [])`; a `normalize_team_name`-keyed reconcile resolves the SAME request to `🏆 Team Hickey -> [1, 2, 3]`. So the raw `league_rosters.get(team_name, [])` at `trade_finder_service.py:34` returns `[]` whenever the client name lacks the Yahoo key's emoji/whitespace → empty `user_roster_ids`.
  - (b) `all_team_totals=None` early-return at `src/trade_finder.py:1895` — CONFIRMED INDEPENDENT. Ran the REAL `find_trade_opportunities` against a non-empty 5-player pool with a valid non-empty `user_roster_ids=[1,2,3]` + non-empty `league_rosters`, but `all_team_totals=None` → returned `0` suggestions. So `all_team_totals=None` (passed at `trade_finder_service.py:40`) forces `[]` on its own, even after (a) is fixed. BOTH must be fixed.
  - Local DB context: `load_player_pool()` = 9888 rows (real 26 MB pool present); `load_league_records()` = 12 teams with cols `[team_name, wins, losses, ties, win_pct, points_for, points_against, streak, rank, updated_at]`. `load_league_rosters()` (the static table) is EMPTY in this checkout — Yahoo rosters arrive at runtime via `get_yahoo_data_service().get_rosters()`, so the live emoji-keyed mismatch is reproduced via the documented key, not the static DB.
  - Live `GET /api/trade-finder?team_name=Team+Hickey` (before fix): not hit directly this session (the two-mode root cause is fully proven locally); the live "No trade ideas yet" empty state is the user-visible symptom of (a)+(b).
- **Task 6 live-verify (after fix):**
  - Finder pipeline (real 26 MB pool, no live Yahoo): the fix flows correctly end-to-end — `_resolve_user_roster("Team Hickey", {...})` resolves the synthetic "🏆 Team Hickey" emoji key → the real 14-id roster (NOT []); `get_all_team_totals` computes totals for all 4 teams; `find_trade_opportunities(all_team_totals=<non-None>)` runs (no early-return). The synthetic all-top-ranked roster yields 0 engine suggestions (the engine's fairness/acceptance/elite-floor filters reject trades where the partner doesn't also gain — engine working as designed, NOT a regression). The two compounding bugs (a)+(b) that GUARANTEED [] are removed.
  - `grade` / `partner_record` / `category_impacts` populated on REAL DB data? YES. Fed `_build_suggestions` an engine-shaped dict + real `load_league_records` (12 teams): `grade='B+'` (read off the dict), `partner_record='11-2-0 · 1st'` (real records, ordinal + emoji-tolerant match to "Over the Rembow"), player refs resolved by name, `category_impacts`=12 cats. With a real differing-stat swap (give 1-HR player IN roster, get a 39-HR bat): nonzero correctly-signed SGP deltas `R -0.25, HR -0.077, RBI -0.219, AVG +0.087, OBP +0.741` (give-player-not-in-roster → before==after → all-zero, which is correct; the engine only emits trades where giving_ids ARE rostered, so live impacts are always meaningful).
  - Compare `/api/compare?ids=…`: router verified — `require_login`-gated only (dormant when Clerk off), THIN delegate to `CompareService.compare(player_ids)`, parses CSV ids defensively. (Not hit over HTTP this session — no live Yahoo in the headless main checkout; code path confirmed.)
  - Build `/api/trade/evaluate`: `TradeEvaluateRequest.enable_mc` defaults `False` (`api/contracts/trade.py:14`); frontend `web/src/lib/trades-data.ts:210` sends `enable_mc: false`; `BuildPanel.tsx` has NO enable_mc reference → no UI path flips MC on → no 45s synchronous-MC hang. Router is Pro-gated (dormant) + `resolve_required_team`.
  - Pro-gate dormant confirmed: `api/gating.py::require_pro` returns at line 42-43 when `not stripe_enabled()` (= `billing_env_configured()` False → Stripe unset on the free beta). The Finder + Build `Depends(require_pro)` are no-ops → friends never 402.
  - Deferred: per-suggestion playoffDelta (a `simulate_trade_playoff_delta` sim per card is too slow — same call the finder's `enable_title_odds_rerank` gates behind). `TradeRec.playoffDelta` stays mock-only; the card renders it only when present.
  - No new bug surfaced in Task 6 → no extra commit (verification only).
