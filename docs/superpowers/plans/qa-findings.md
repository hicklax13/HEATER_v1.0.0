...
> The disconnected QA re-run's one suite-visible F (test #18 FA renders, under the guard) is a SEPARATE, guard-specific failure mode (exception / st.error on the empty-FA-cache member path) — under investigation; the standard CI suite (5,110) stayed green throughout.
