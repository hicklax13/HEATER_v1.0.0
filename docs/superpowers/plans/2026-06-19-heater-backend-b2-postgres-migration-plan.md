...
> **Cross-track note (2026-06-21, from the backend-data session — include in this port):** the bootstrap gained a NEW phase `_enrich_mlb_ids` (`src/data_bootstrap.py:1461`, dispatched ~`:3641`) + the operator tool `scripts/backfill_player_mlb_ids.py`, both backed by raw `get_connection()`: a `players` ⋈ `league_rosters`/`ownership_trends` SELECT (the relevant-subset target, ~`:1496`) and `UPDATE players SET mlb_id = ? WHERE player_id = ? AND (mlb_id IS NULL OR mlb_id = 0)` (~`:1544`). Port these (+ the script's `_write`/`_existing_*`) here. **Preserve the phase's read→resolve→write split** — it deliberately does NOT hold a connection across the statsapi name→mlb_id lookups in `src/player_id_resolver.py`; a naive port that wraps the whole phase in one engine transaction would re-introduce a long-held write lock (the exact SFH-M1 class we fixed for pvb_splits). `src/player_id_resolver.py` itself is pure (no DB) — nothing to port there.
...
6. **Dangling `ai_*` FK once FKs are ENFORCED** (cross-track, 2026-06-21, from the Bubba/CEO track) — `ai_provider_keys` / `ai_conversations` / `ai_usage_ledger` declare `user_id ... REFERENCES users(user_id)` but it is UNENFORCED today (`get_connection` doesn't `PRAGMA foreign_keys=ON`). Bubba (the API chat) writes those tables with a NAMESPACED `user_id` (`1_000_000_000 + AppUser.id`, set in its `chat_service`) that has NO matching `users` row — fine while unenforced, but the moment B2 turns FKs ON (the deferred `op.create_foreign_key` step the baseline omits), those API-written rows violate the `→users` FK. *Mitigation:* do NOT blindly enforce the `ai_*.user_id→users` FK in B2; the clean fix is **moving chat persistence into the api-owned store (api_state.db→Postgres) keyed by `AppUser.id`** (drops the offset AND the API→`draft_tool.db` write) — logged as a B-phase follow-up by the Bubba track; coordinate before enforcing.
