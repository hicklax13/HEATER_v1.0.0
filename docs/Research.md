...
> **Status (2026-05-19):** Most original P1/P2/P3 gaps have shipped. Each item below is
> annotated with **✅ SHIPPED** + module reference when delivered, or **❌ STILL GAP**
> if open. Per-feature shipped status was reconciled against PRs #18-#47.
>
> **Current remaining gaps** (down from 20 → 6):
> - P1.2 Multi-Platform League Sync (ESPN/CBS/Sleeper) — Yahoo-only by design choice
> - P1.5 Keeper/Dynasty League Support — out of scope for current FourzynBurn league
> - P1.6 Auction/Salary Cap Calculator — out of scope (snake-draft format)
> - P3.16 Draft Order Generator — minor utility, lower priority
> - P3.17 Auto-Swap IL Players — partial via `il_manager.py`; auto-action requires Yahoo write API
> - P3.18 Multiple League Support — partial via `league_manager.py`/`league_registry.py`

> **Date:** 2026-03-19 (initial), 2026-05-19 (shipped-status reconciliation)
...
| **Closer Depth Chart Visualization** | **page 3 Closer Monitor** | **30-team grid + gmLI trust + opener detection + committee risk scores (shipped pre-#18)** |
| **Streaming Pitcher Daily Recommendations** | **Lineup Optimizer Streaming tab + `src/optimizer/streaming.py`** | **DCV scoring + two-start fatigue + dynamic SGP threshold (shipped pre-#18)** |
| **Schedule Grid / Weekly Planner** | **`src/schedule_grid.py`** | **7-day visual with matchup-quality color coding (shipped pre-#18)** |
| **League Power Rankings** | **`src/power_rankings.py`** | **Per-team SGP totals + strength/weakness breakdown (shipped pre-#18)** |
| **Pick Predictor (Survival Probability UI)** | **`src/pick_predictor.py`** | **Decay curve "available at pick N: X%" surface (shipped pre-#18)** |
| **Player Tags** | **`src/player_tags.py`** | **Per-user Sleeper/Target/Avoid/Breakout/Bust badges (shipped pre-#18)** |
| **Cheat Sheet Export** | **`src/cheat_sheet.py`** | **Styled HTML/PDF cheat sheet with tier customization (shipped pre-#18)** |
| **Prospect Rankings** | **`src/prospect_engine.py`** | **FanGraphs Board + MiLB stats + 40-man call-up alerts (shipped pre-#18)** |
| **Projected Season Standings** | **page 6 League Standings → "Season Projections" tab** | **MC projections + magic numbers + per-team projected W/L (shipped PR #44)** |
| **Points League Projections** | **`src/points_league.py`** | **Yahoo/ESPN/CBS scoring presets + custom weights (shipped pre-#18)** |
| **Scoring / Points Leaders** | **page 17 Leaders (7-tab)** | **Leaders / Value / Breakouts / Prospects / Hot / Cold / Sell-High (shipped PR #43)** |
| **"Who Should I Start?" Quick Compare** | **page 16 Player Compare** | **Z-score normalization + marginal SGP team impact + radar charts (shipped pre-#18)** |
| **IL Manager (partial Auto-Swap)** | **`src/il_manager.py`** | **IL detection + replacement recommendation surface (Yahoo write requires user action)** |
| **ECR Integration** | **`src/ecr.py` (6-source aggregation)** | **FantasyPros + CBS + Yahoo + ESPN + Razzball + RotoBaller via Trimmed Borda; outperforms FP's single-platform ECR** |
| **Live Draft Assistant (partial)** | **`src/live_draft_sync.py`** | **Yahoo Draft API polling + auto-detect picks (partial — manual entry still primary path)** |
...
#### 1. ✅ SHIPPED — Expert Consensus Rankings (ECR) Integration
*HEATER has 6-source ECR aggregation via Trimmed Borda — see `src/ecr.py`. Goes beyond FantasyPros' single-platform expert pool.*
...
#### 2. ❌ STILL GAP — Multi-Platform League Sync (ESPN, CBS, Sleeper, Fantrax)
*Yahoo-only by design choice (FourzynBurn is a Yahoo league); not a regression-risk gap, but blocks broader adoption.*
...
#### 3. ⚠️ PARTIAL — Live Draft Assistant with Real-Time Sync
*Yahoo draft polling lives in `src/live_draft_sync.py`. UX integration with the draft page is still partial.*
...
#### 4. ✅ SHIPPED — Projected Season Standings
*Page 6 League Standings → "Season Projections" tab (PR #44). MC season totals + magic numbers + projected W/L per team.*
...
#### 5. ❌ STILL GAP — Keeper / Dynasty League Support
*Out of scope for current FourzynBurn (single-season redraft). Would require age curves + prospect valuation + surplus-value computation.*
...
#### 6. ❌ STILL GAP — Auction / Salary Cap Calculator + Simulator
*Out of scope (FourzynBurn is snake-draft format). Would add auction DraftState variant + bidding AI.*
...
#### 7. ✅ SHIPPED — Printable/Exportable Cheat Sheets
*See `src/cheat_sheet.py`. Styled HTML/PDF generation with custom tier annotations.*
...
#### 8. ✅ SHIPPED — Player Tags (Sleeper / Target / Avoid)
*See `src/player_tags.py`. Persistent per-user tags surfaced as badges on hero card + search results.*
...
#### 9. ✅ SHIPPED — Closer Depth Chart Visualization
*Page 3 Closer Monitor. 30-team grid + gmLI trust + K% skill decay alerts + opener detection.*
...
#### 10. ✅ SHIPPED — Streaming Pitcher Daily Recommendations
*Lineup Optimizer Streaming tab + `src/optimizer/streaming.py`. DCV scoring + two-start fatigue + dynamic SGP threshold.*
...
#### 11. ✅ SHIPPED — Schedule Grid / Weekly Planner View
*See `src/schedule_grid.py`. 7-day visual with matchup-quality color coding from matchup_planner ratings.*
...
#### 12. ✅ SHIPPED — League Power Rankings
*See `src/power_rankings.py`. Per-team SGP + strength/weakness breakdown + sortable leaderboard with drill-down.*
...
#### 13. ✅ SHIPPED — Pick Predictor (Survival Probability UI)
*See `src/pick_predictor.py`. Player-searchable lookup showing "Available at pick N: X%" decay curves.*
...
#### 14. ✅ SHIPPED — Points League Projections
*See `src/points_league.py`. Yahoo/ESPN/CBS scoring presets + custom-weight support.*
...
#### 15. ✅ SHIPPED — Prospect Rankings
*See `src/prospect_engine.py`. FanGraphs Board + MiLB stats + 40-man call-up alerts. Page 17 Leaders has dedicated "Prospects" tab.*
...
#### 16. ❌ STILL GAP (low priority) — Draft Order Generator
*Minor utility — most leagues handle via Yahoo platform. `src/draft_order.py` has snake-order math, no UI.*
...
#### 17. ⚠️ PARTIAL — Auto-Swap Inactive/IL Players
*See `src/il_manager.py` for IL detection + replacement recommendation surface. Auto-execution requires Yahoo write API access (which user must approve separately).*
...
#### 18. ⚠️ PARTIAL — Multiple League Support
*See `src/league_manager.py` + `src/league_registry.py`. Schema accommodates multiple leagues; UI selector + per-league DraftState integration is incomplete.*
...
#### 19. ✅ SHIPPED — "Who Should I Start?" Quick Compare
*Page 16 Player Compare provides head-to-head with category fit + schedule strength + SGP breakdown. Faster than running full optimizer.*
...
#### 20. ✅ SHIPPED — Scoring / Points Leaders
*Page 17 Leaders. 7 tabs: Leaders / Value / Breakouts / Prospects / Hot / Cold / Sell-High (PR #43 fold of Trends into Leaders).*
...
## Gap Summary (2026-05-19 reconciliation)

| Priority | Original | ✅ Shipped | ⚠️ Partial | ❌ Still Gap |
|----------|----------|-----------|------------|--------------|
| **P1: High-Impact** | 6 | 2 (ECR, Projected Standings) | 1 (Live Draft Assist) | 3 (Multi-Platform, Keeper, Auction) |
| **P2: Medium-Impact** | 7 | 7 (all) | 0 | 0 |
| **P3: Nice-to-Have** | 7 | 4 | 2 (Auto-Swap IL, Multi-League) | 1 (Draft Order Gen) |
| **P4: Out of Scope** | 6 | — | — | 6 (intentionally excluded) |
| **TOTAL** | **20 gaps** | **13 shipped** | **3 partial** | **4 still gap** |

Original "Recommended Implementation Order" below is preserved for historical context but is no longer the active roadmap.
