...
- **P90 ERA/WHIP inversion** — `compute_percentile_projections()` negates the z-multiplier for inverse rate stats (ERA, WHIP). P90 = optimistic = lowest ERA, P10 = pessimistic = highest ERA. Uses `z_eff = -z if col in {"era", "whip"} else z`.
- **Maximin LP excludes inverse cats** — `maximin_lineup()` auto-filters out `INVERSE_CATS` (L, ERA, WHIP) because their SGP scale factors are incompatible with counting stats and dominate the LP floor. Only non-inverse categories participate in the maximin optimization.
- **Kalman prior_var = process variance only** — In `_apply_kalman_filter()`, `prior_var = proc_var` (not `proc_var + obs_var`). Adding observation variance to the prior double-counts noise and over-trusts observed data.
- **Opponent pick probability is additive blend** — `simulation.py` normalizes ADP, need, and history weights into separate probability distributions before blending: `combined = w_adp * adp_dist + w_need * need_dist + w_hist * hist_dist`. The old multiplicative form collapsed to near-zero for low-weight players.
- **Pick predictor `_SIGMA = 10.0`** — Matches `simulation.py`'s default ADP standard deviation. Previously was 12.0 which made survival curves too optimistic.
- **ECR consensus uses trimmed Borda Count** — `refresh_ecr_consensus()` calls `_compute_player_consensus()` per player (trimmed mean: drops min/max when >= 4 sources) instead of `DataFrame.mean()` across rank columns.
- **Prospect wOBA proxy** — `_age_level_performance()` uses `woba_proxy = obp * 0.92 + slg * 0.47 - 0.08` (proper linear weights) instead of the previous `(obp * 1.2 + slg * 0.8) / 2` which overweighted OBP.
- **Scenario generator clips non-negative stats** — `generate_stat_scenarios()` applies `np.maximum(0, ...)` to counting stats (R, HR, RBI, SB, W, SV, K) after correlated sampling to prevent physically impossible negative values.
- **Scenario generator Cholesky fallback** — Wraps `np.linalg.cholesky()` in try/except; falls back to identity matrix if correlation matrix isn't positive-definite after nearest-PD correction.
...
- **Systematic code reviews:** Seven rounds of full codebase debugging (172 bugs fixed): Round 1 (10 bugs — data pipeline, Yahoo API, lineup optimizer, CI), Round 2 (9 bugs — MC rate stats, regime detection, bellman DP, convergence, lineup optimizer UI, trade analyzer HTML), Round 3 (8 bugs — SGP denominators, survival gauge, injury persistence, percentile volatility, player pool duplicates), Round 4 (3 bugs — connection leak, player_name alias, export buttons), Round 5 (3 bugs — standings long-format parsing, maximin display format, emoji in H2H subheader), Round 6 (116 bugs — 7 new in-season modules: trade_value.py, two_start.py, start_sit.py, matchup_planner.py, waiver_wire.py, trade_finder.py, draft_grader.py — time-decay ordering, G-Score/replacement level mismatch, confidence tier logic, thread safety, loss aversion formula, STEAL/REACH AND-logic, category balance CV→std, park factor inversion for pitchers, rate stat scaling, sustainability interpolation slopes), Round 7 (23 bugs — comprehensive mathematical algorithm audit across all ~200 algorithms in 18 source files: P90 ERA/WHIP inversion, Kalman prior variance, maximin inverse cat exclusion, opponent pick probability blend, ECR trimmed Borda Count, closer bonus delegation, playoff probability linear decay, L category handling, scenario generator non-negative clipping, Cholesky fallback, prospect wOBA proxy, Bayesian stabilization points, two-start IP estimation, matchup planner per-game starts). All pushed to master, all CI green.
