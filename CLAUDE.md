...
2. **In-Season Management** (`pages/`) — 15 Streamlit pages: team overview, draft simulator, trade analysis (with trade finder + deep analysis + 3-team trades), player comparison, free agent rankings, lineup optimizer, weekly dashboard, trade values, waiver wire, category tracker, trends, playoff odds, bullpen monitor, punt analyzer, weekly recap. Powered by MLB Stats API + pybaseball + optional Yahoo Fantasy API. All pages share centralized single-theme system (light mode only) with glassmorphic design, orange sidebar branding, and bold Heater identity.
...
pages/
  ...
  7_Weekly_Dashboard.py — In-season: weekly hub with matchup ratings, start/sit recommendations, streaming candidates, upcoming schedule, category heatmap
  8_Trade_Values.py     — In-season: sortable trade value chart (0-100), contextual overlays, position scarcity analysis, "What Can I Get?" tool
  9_Waiver_Wire.py      — In-season: add/drop recommendations with net SGP delta, sustainability scores, category impact, position pill filters
  10_Category_Tracker.py — In-season: per-category standings tracker, trend arrows (up/down/flat), weekly projections, category gap visualization
  11_Trends.py          — In-season: player trend detection (hot/cold streaks) vs projections, actual-vs-projected delta, trend alert badges
  12_Playoff_Odds.py    — In-season: Monte Carlo playoff simulator (10K sims), rank distribution histogram, scenario analysis
  13_Bullpen.py         — In-season: bullpen monitor with closer/setup/hold hierarchy, workload tracking, handedness splits
  14_Punt_Analyzer.py   — In-season: punt strategy analyzer, category cost/benefit analysis, optimal punt combinations by standings position
  15_Weekly_Recap.py    — In-season: weekly matchup recap with category wins/losses, suggested moves for next week
src/
  ...
  playoff_sim.py        — Monte Carlo playoff simulator: 10K season simulations, strength-of-schedule adjustments, rank distribution, scenario analysis with what-if trades
  trend_tracker.py      — Player trend detection: actual-vs-projected delta computation, hot/cold streak classification, trend labeling (HOT/COLD/NEUTRAL), minimum sample size guards
...
- **`live_stats._pct()` returns proportions** — `_pct()` returns 0.0–1.0 (e.g., 0.223), NOT percentages (22.3). League average defaults match: `k_pct: 0.223`, `bb_pct: 0.085`. The `two_start.py` module has its own `fetch_team_batting_stats()` that also returns proportions.
- **`live_stats.fetch_team_batting_stats` is thread-safe** — Uses `threading.Lock` for both cache reads and writes. Consistent with `two_start.py`'s thread-safe cache pattern.
- **`trade_value.compute_trade_values()` does NOT mutate config** — Creates a new `LeagueConfig()` copy when updating SGP denominators from standings. Previous bug mutated the shared config object.
- **`trend_tracker` hitter detection** — Uses `pd.notna()` check before `int()` cast on `is_hitter` field. Avoids Python falsy `0 or 1 → 1` treating pitchers (`is_hitter=0`) as hitters.
- **`rank_streaming_candidates()` expects `list[dict]`** — Not a DataFrame. Callers must use `.to_dict("records")` when passing from a DataFrame source.
- **`evaluate_trade()` market_values structure** — Returns `{player_id_str: {market_price, demand, max_bidder, max_bid, valuations}}`, NOT a flat dict. UI must iterate over per-player entries.
- **`playoff_sim.py` simulation count** — Default 10K sims. `simulate_season()` returns rank distribution dict with probabilities. Uses SOS (strength of schedule) adjustments.
- **`trend_tracker.py` minimum sample size** — Requires 30+ PA for hitters, 10+ IP for pitchers before computing trend deltas. Below threshold returns NEUTRAL.
- **New pages require league data** — Pages 7-15 all follow the standardized pattern: check for `league_rosters` data, show the standard "No league data loaded" warning when missing.
- **`start_sit.py` reliever detection** — Uses `sv > 3 or ip < 80` to identify relievers for per-appearance IP calculation. SP uses IP/5.5 (default IP per start).
- **`two_start.py` ip_per_start estimation** — Estimates actual starts from total IP: `estimated_starts = round(ip / 5.5)`, then `ip_per_start = ip / estimated_starts`. Not a fixed 5.5 default.
...
- **Test files:** 61 test files across draft engine, trade engine (Phase 1-6), lineup optimizer (10 files), draft recommendation engine (5 files), gap closure (14 files), in-season analytics (7 files), in-season, analytics, data pipeline, bootstrap, integration, backtesting, and math verification
...
- **Systematic code reviews:** Seven rounds of full codebase debugging (168 bugs fixed): Round 1 (10 bugs — data pipeline, Yahoo API, lineup optimizer, CI), Round 2 (9 bugs — MC rate stats, regime detection, bellman DP, convergence, lineup optimizer UI, trade analyzer HTML), Round 3 (8 bugs — SGP denominators, survival gauge, injury persistence, percentile volatility, player pool duplicates), Round 4 (3 bugs — connection leak, player_name alias, export buttons), Round 5 (3 bugs — standings long-format parsing, maximin display format, emoji in H2H subheader), Round 6 (116 bugs — 7 new in-season modules: trade_value.py, two_start.py, start_sit.py, matchup_planner.py, waiver_wire.py, trade_finder.py, draft_grader.py — time-decay ordering, G-Score/replacement level mismatch, confidence tier logic, thread safety, loss aversion formula, STEAL/REACH AND-logic, category balance CV→std, park factor inversion for pitchers, rate stat scaling, sustainability interpolation slopes), Round 7 (19 bugs across 14 files — 9 new pages + 2 new src modules + modified pages/src: live_stats _pct() percentage→proportion, start_sit reliever stats inflated 5x, trade_value shared config mutation, trend_tracker pitcher misclassification, Weekly Dashboard DataFrame→list[dict] crash, Trade Analyzer market_values dict-of-dicts, start_sit missing pitcher park factors, live_stats thread-unsafe cache, ui_shared heatmap IP key case mismatch, Trade Analyzer pre-fill failure, Trade Values rep_levels type error, two_start ip_per_start no-op formula, deprecated use_container_width, dead code removal). All pushed to branch, all CI green.
