...
**Deferred (not bugs — do not re-flag):** Phase 7 roadmap (Apply-to-Yahoo write-back, multi-player trades, save/load, watchlists, export/share, monolith refactors, async optimization, mobile, onboarding, dark mode, accessibility, thin-client) — spec'd in the plan, intentionally not built. Small follow-ups: `render_matchup_ticker` (the compact top ticker bar) in `ui_shared.py` still gates on `yahoo_connected` (the main Matchup Pulse card is cache-served and works); the Trade Finder "Analyze in Trade Analyzer" deep-link stashes `st.session_state["_tf_prefill"]` but the Trade Analyzer read-side isn't wired yet; a few `st.info`→`render_empty_state` swaps (plan task 5.5).
