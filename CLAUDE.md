...
**★ Sub-project B status (updated 2026-06-19) — backend API surface BUILT + on `master`, dormant:** **16 `/api/*` endpoints** (+ `/healthz`) over the **UNCHANGED** Python engines, shipped via 8 TDD slices (plans `docs/superpowers/plans/2026-06-17-heater-backend-api-slice{1..8}.md`). The live Streamlit app is untouched — the `api/` code is inert until cutover (Sub-project D). Endpoints: **13 read** (`me/team`, `free-agents`, `lineup/optimize`, `standings`, `closers`, `leaders`, `matchup`, `streaming`, `punt`, `trade/evaluate`, `trade-finder`, `compare`, `databank`) + **2 write** (`POST /api/lineup/set`, `POST /api/transactions/add-drop` — slice 7, over the already-graceful Yahoo write methods; all outcomes HTTP 200 with status in `MutationResult.ok`) + **1 draft** (`POST /api/draft/recommend` — slice 8, **stateless**: client holds `config`+`pick_log`, server replays to rebuild `DraftState`). **Proven per-slice pattern (reuse for any new endpoint):** contract (`api/contracts/`) → service seam (`api/services/*_service.py`, the ONE place importing `src/`) → thin logic-free router (`api/routers/`, AST-guarded by `tests/api/test_no_logic_in_routers.py`) → DI provider in `api/deps.py` → fake-service dependency-override test (basename MUST be `tests/api/test_api_*.py`) → mount in `api/main.py` → regen `api/openapi.json` (snapshot-guarded by `test_openapi_contract.py`; **`fastapi==0.137.1` + `httpx==0.28.1` are PINNED** because FastAPI auto-generates the OpenAPI error schemas, so an unpinned drift fails the snapshot test with no code change). **STILL TO DO (each its own slice/decision):** AI-opponent draft-sim endpoint (`POST /api/draft/simulate-picks` — needs extracting `auto_pick_opponents` from `pages/20_Draft_Simulator.py` into `src/simulation.py`, so it touches the live page); auth-gating the write endpoints (Clerk) + the Yahoo `fspt-w` write scope (app is registered read-only); and the **owner-deferred infra B2 Postgres → B3 Redis/Arq workers (async the MC/optimize paths) → B4 Clerk auth + multi-tenancy → B5 strangler-fig cutover** (do NOT start without an agreed plan — these touch the LIVE data path). The CMO frontend (`web/`) can wire against the full contract now.
