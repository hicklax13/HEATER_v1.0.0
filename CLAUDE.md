...
A fantasy-baseball draft assistant + in-season manager for a 12-team Yahoo Sports H2H Categories snake-draft league. Powered by Monte Carlo simulation, Bayesian projection updating, and LP-constrained lineup optimization. **Now shipped two ways over the SAME Python engines:** the original **Streamlit app** (App A — kept as a strangler-fig fallback) AND the live **Next.js/React + FastAPI product** (App B / Vercel) that the league actually logs into — a private **12-friend beta is LIVE** (see ★ NORTH STAR / MASTER GOAL below). **2026 MLB season is active; the app is in in-season mode** — draft completed, all components running against live league + Yahoo data.

> ## ★ NORTH STAR / MASTER GOAL (updated 2026-06-26) — take HEATER PUBLIC + COMMERCIAL as a real, paid product
> **The ultimate goal is now a PUBLIC COMMERCIAL LAUNCH: a monetized, general-public fantasy-baseball product that anyone can sign up for and pay to use.** Every future session treats this as the destination behind feature and architecture decisions.
>
> - **★ WHERE WE ARE NOW (2026-06-26) — the re-platform is DONE and a private friends beta is LIVE.** HEATER ships as a **Next.js/React frontend on Vercel** (https://heater-v1-0-1.vercel.app) + a **FastAPI API on Railway** (App B `celebrated-respect`) over the UNCHANGED Python engines, with **Clerk auth** (dev instance), **Stripe billing BUILT but DORMANT** (the 12 friends are free), and **Bubba** (the AI assistant) live across all 6 providers. M0–M3 are complete: all 14 surfaces are live React pages on real Yahoo data; per-user team resolution + admin team-assignment work. The original **Streamlit app stays as a strangler-fig fallback** (App A). The 2026-06-26 user-test remediation is shipped + live-verified (memory [[project_user_test_2026_06_26]], `docs/testing/2026-06-26-comprehensive-user-test-report.md`); the league-launch playbook is `docs/deployment/beta-league-onboarding.md`.
>
> - **★★ THE NEXT TASK (do this FIRST, before building anything): SPEC + PLAN the multiphase "public commercial launch" program.** Use **brainstorming** (resolve scope / sequencing / pricing / positioning WITH the owner), then **writing-plans** (one plan file per phase under `docs/superpowers/plans/`), then **dispatching-parallel-agents** to execute — and reach for EVERY applicable skill / plugin / MCP (vercel, railway, stripe, clerk/auth, legal, design/frontend, chrome-devtools, etc.). The plan must cover the public-launch hard requirements (largely the deferred M4/M5 infra + go-to-market):
>   - **Multi-tenancy (B4):** real per-user leagues — connect Sleeper/Yahoo/ESPN/CBS + manual import, per-user OAuth, `league_id` tagging + tenant isolation. Design LOCKED (`docs/superpowers/specs/2026-06-21-heater-b4-multi-tenancy-tenant-resolution-design.md`); build is owner-gated + needs a real Postgres.
>   - **Infra for scale (B2 Postgres + B3 workers):** replace the single-replica SQLite sole-writer with **Postgres**, and offload heavy per-request compute (e.g. the 17–59s `/api/me/team`) to **Redis/Arq background workers**. B2.0/B2.1 shipped + dormant.
>   - **Auth + billing GO-LIVE:** Clerk **production** instance (requires a **CUSTOM DOMAIN** — a `*.vercel.app` can't host the DNS records) + Stripe **live** (the tier gating is built, currently dormant).
>   - **Legal + trust:** ToS / privacy / data-source compliance (`docs/legal/`), account management, data export/deletion.
>   - **Go-to-market:** public landing + marketing + SEO, onboarding for strangers (not a known 12-person league), final pricing/packaging, support.
>
> - **Maintenance model (unchanged):** from launch day 1 it is just the owner (Connor) + Claude until revenue funds hires — bias every decision toward solo-operator-plus-AI maintainability and low operational overhead. The Combustion design language and the Python engine are preserved through all of this.

> ## ★ SESSION HANDOFF (2026-06-27) — the public-launch program is now SPEC'd, PLANNED, and partially EXECUTED
> **The North Star's "THE NEXT TASK" (spec + plan the public-commercial-launch program) is DONE.** The program now exists and is underway. (The "do this FIRST: SPEC + PLAN" directive above is COMPLETE — superseded by this handoff.)
> - **Master spec (16 phases):** `docs/superpowers/specs/2026-06-26-heater-public-commercial-launch-program-design.md`. Owner-locked decisions: full **Codex 100/100** bar before any public dollar; **internal-audits-now** (paid external pen-test / statistician / a11y / lawyer = an owner-arranged step before the public flip); **ALL formats** (H2H Cats + Roto + Points) + **ALL providers** (Yahoo/ESPN/CBS/Sleeper + manual); **tiered (Free/Pro/Premium) + usage-metered** pricing. Memory: [[project_public_launch_program]].
> - **Evidence registry = the live scoreboard:** `docs/launch/evidence_registry.yaml` (40 reqs; run `python -m scripts.launch.evidence_registry --summary`). `docs/launch/README.md` indexes the launch docs.
> - **Shipped this session (all on `origin/master`):** **Phase 0 complete** (0a baseline-freeze tool + evidence registry + CI guards [router-mount, OpenAPI↔TS]; 0b API contract foundation — error envelope `api/errors.py`, correlation-ID `api/request_context.py`, OpenAPI BearerAuth, idempotency + async-job contracts). **Phase 1 internals** (`docs/launch/source_inventory.md` + `data-licensing-options.md` + `legal-policy-gap-assessment.md` + `docs/legal/AI_DISCLOSURE.md`; privacy-ops DESIGNED, impl deferred to post-Postgres). **Additive hardening** (`api/security_headers.py`; `api/health.py` = /livez + /readyz; bandit SAST CI gate; `docs/launch/ai-safety.md`). **Phase 4 slice 1 — Points scoring engine** (`src/points_scoring.py` — standalone, never touches the category engine; 13 TDD tests; two-way Ohtani validated #1@656pts).
> - **★★ CRITICAL launch finding:** HEATER's core data (MLBAM/FanGraphs/FantasyPros, scraped) is **commercial-use-PROHIBITED as built**. Fix = buy the same facts from a **COMMERCIAL feed** (stats+names are public-domain per CBC v. MLBAM; MySportsFeeds/SportsDataIO ~low-hundreds-$/mo, NOT enterprise) + in-house Marcel projections + drop scraped sources. Caveat: player headshots/team logos = separate likeness/trademark. See `docs/launch/source_inventory.md` + `data-licensing-options.md`.
> - **⚠️ Owner-gated remainder (the critical path):** Phase 1 licensing/counsel; **Phase 2 Postgres — needs a provisioned DB (owner skipped Neon "for now" 2026-06-26)**; then Phase 3 tenancy. Everything from Phase 2 chains through these.
> - **⚠️ Parallel CMO track:** a separate session works the frontend/integration lane (branch `feat/optimizer-streaming-startsit-trades-bubba`). Reconcile via `git pull --no-rebase`; do NOT disturb their branch/uncommitted work. The launch-program (CEO track) stays in standalone/backend layers to avoid collision.
>
> ### ★ NEXT SESSION — START HERE (the next agent should run this task on a fresh chat)
> **Task — continue the public-launch program, Phase 4 slice 2 (extend the standalone scoring valuation):**
> 1. Read `docs/launch/evidence_registry.yaml` + memory [[project_public_launch_program]] to confirm state, and `git pull --no-rebase origin master` to reconcile the parallel CMO track.
> 2. Use **`superpowers:brainstorming`** to resolve slice 2 WITH the owner — choose between **(a) Rotisserie valuation/standings** (breadth: completes all-formats valuation; reuse `SGPCalculator`/`standings_engine`) and **(b) points value-over-replacement** (depth: positional scarcity makes the Points engine decision-useful, mirroring HEATER's category VORP). Stay in the **standalone valuation layer** (a new `src/*.py` like `src/points_scoring.py`) — do NOT touch the optimizer/FA/trade integration layer (the parallel CMO track owns it).
> 3. Then **`superpowers:writing-plans`** → **`superpowers:executing-plans`** (TDD, one focused module + tests), add a `P4-*` row to `docs/launch/evidence_registry.yaml`, push to `master`.
> Honor Connor's brevity preference; build to the internal-now / certify-later bar; reach for any applicable skill/MCP the work calls for.
...
**★ Updated 2026-06-26 — friends beta LIVE-VERIFIED + the MASTER GOAL pivots to PUBLIC COMMERCIAL LAUNCH (see the rewritten ★ NORTH STAR / MASTER GOAL up top).** A full user-test + remediation shipped + live-verified on production (memory [[project_user_test_2026_06_26]]; `docs/testing/2026-06-26-comprehensive-user-test-report.md`): **M-1** live FA value=0 FIXED via a **Marcel projection fallback** (FanGraphs 403s from Railway's datacenter IP → compute projections locally from `season_stats`; new `src/marcel_bootstrap.py`; live App B logs show "Marcel fallback: wrote N projection rows" → value non-zero); **M-2/M-3** hard-load bounce FIXED (`web/src/lib/api/client.ts::authToken()` now awaits `window.Clerk.loaded` before deciding there's no token; the league-wide routers are `require_login`-only so they render for unlinked viewers); **M-4** route titles via per-route server segment `layout.tsx` metadata (deleted the client `DocumentTitle` hack); **L-6/L-8** minor; **Stripe webhook event-id dedup** (dormant). **Bubba's 6 managed AI keys are wired + each live-tested** — per-provider env `HEATER_AI_ADMIN_KEY_<PROVIDER>` (anthropic/openai/gemini/deepseek/xai/openrouter; `src/ai/keys.py`); **the key VALUES live in gitignored `.env` + `C:\Users\conno\.heater_ai_keys` + Railway App B, NEVER in git** (memory [[reference-ai-provider-keys]]). **Connor's account is linked to 🏆 Team Hickey; the other 11 leaguemates are linked LAST, after each signs in** (`docs/deployment/beta-league-onboarding.md`). **★ With the friends beta done, the active strategic line is the NORTH STAR's "THE NEXT TASK": SPEC + PLAN the public-commercial-launch program** (multi-tenancy / Postgres / workers / Clerk-prod / Stripe-live / legal / go-to-market) BEFORE building.

**★ Updated 2026-06-27 — 5-workstream React+API feature SHIPPED to `master` (NOT yet deployed; spec + 5 plans `docs/superpowers/{specs,plans}/2026-06-27-*`; memory [[project_optimizer_streaming_startsit_trades_bubba]]).** Built brainstorm→writing-plans→subagent-driven-development (fresh implementer + per-slice spec+quality review; silent-failure-hunter on WS4). **WS1 Optimizer** (`/optimizer`): `LineupOptimizeRequest.scope ∈ {today,rest_of_week,rest_of_season}` (engine-native — today→daily DCV, week/season→standard LP) + FA-aware (`lineup_service` composes `recommend_fa_moves(ctx)` → new `LineupOptimizeResponse.fa_suggestions: list[FaSuggestion]` in `api/contracts/lineup.py`) + standard-mode now passes the live matchup; frontend regrouped by **current Yahoo slot** (batters/pitchers split, decision colors, cols Slot·Player·Eligibility·Value·Matchup·Decision) + "Recommended pickups" panel. **WS2 Streaming** (`/streaming`): date strip today→+7 (mirrors Probables, `?date=`); the board was ALREADY matchup-weighted via `build_optimizer_context` → real work was surfacing `StreamingResponse.urgency` (={} when no live matchup, else the neutral all-0.5 misleads). **WS3 NEW `/start-sit` page + nav tab** (the user's headline ask): `POST /api/start-sit/compare` (≤6 players, `scope`, ranked start/sit verdict bounded by open slots, each candidate's `eligible_slots` from real Yahoo positions vs league slots C/1B/2B/3B/SS/3×OF/2×Util/2×SP/2×RP/4×P) + `POST /api/start-sit/optimize` (Path-B self-contained LP fill — `lineup_service` has no candidate-injection hook); engine cap 4→6 in `src/start_sit.py`. Reviews caught a **Muncy-DNA join bug** (page joined candidate→verdict by mlbId/name → fixed to authoritative HEATER `player_id`) + a `max(abs)` negative-"0-100" start_score (clamped). Overruled a reviewer `weeks_remaining` must-fix — grep proved all 8 `build_optimizer_context` callers omit it (cross-page consistency). **WS4 Trades Finder** fixed the live "No trade ideas yet" — TWO independent bugs: `trade_finder_service` raw `.get("Team Hickey")` missed emoji key `"🏆 Team Hickey"` + `all_team_totals=None` early-returns `[]`; fixed both + enriched `TradeSuggestion` += grade/partner_record/category_impacts + added `TradeFinderResponse.reason` (ok/team_not_resolved/no_totals/…) + WARNING logs so the empty class can't recur silently. Correct-in-flow; "surfaces real trades" needs live-Yahoo verify on deploy (local `league_rosters` empty). **WS5 Bubba**: 3 read-only explainer TOOLS in `src/ai/tools.py` (`explain_constant`/`list_constants`/`explain_metric` over CONSTANTS_REGISTRY — stream_score exact, dcv/start_score anchored-module, trade_grade is algorithm-derived→0 constants) + `ChatSendRequest.page_context{page,data_json}` folded into the user turn + system prompt (byte-identical when None) + a global frontend `BubbaContext` (`usePageData` publishes `{pageId,data}` on loaded → covers all pages; Bubba auto-attaches a 16KB UTF-8-byte-capped, code-point-safe page snapshot every message, default-on toggle, distinct from the manual tag flow). **Live Streamlit `chat()` stays byte-identical — the frozen-reference drain test is green; tools are purely additive.** Full suite 7149 passed (3 pre-existing env/date failures in `test_ai_keys`/`test_usage_analytics`, NOT this work); `pnpm build` green; all routes mounted. **Deploy (Railway App B + Vercel) is a separate owner step; WS4 "surfaces trades" + Bubba live-send (Clerk-gated) need live verify on deploy.**
