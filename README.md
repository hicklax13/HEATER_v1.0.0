...
Built for the **FourzynBurn** league (12-team H2H Categories, snake draft, 23 rounds, 12 scoring categories, 26-week regular season). ~125,000 lines of Python with ~3,947 tests + ~70 structural-invariant guards. Zero CSV uploads — all data auto-fetched from MLB Stats API, FanGraphs, Yahoo Fantasy, and 10+ other sources on every launch.
...
Pages are ordered for daily workflow (status → daily action → strategy → trades → wire → research → preseason). Consolidated 2026-05-18 from 20 → 13 pages via Section 5 audit (folded Bullpen→Closer_Monitor, Playoff_Odds→League_Standings, Weekly_Dashboard/Weekly_Recap→Matchup_Planner, Trade_Values→Trade_Finder, Waiver_Wire→Free_Agents, Trends→Leaders).
...
| 3 | **Closer Monitor** | 30-team closer depth chart with gmLI trust, K% skill decay alerts, committee risk scores, opener detection + bullpen workload tracking |
| 5 | **Matchup Planner** | Per-category win probabilities (Bayesian + copula), player matchup ratings, per-game detail with umpire/weather/PvB adjustments + weekly recap |
| 6 | **League Standings** | 3-tab: Current Standings, Season Projections, Playoff Odds (top-4 simulation), MC projections, magic numbers, power rankings |
| 10 | **Punt Analyzer** | Punt-category strategy recommender |
| 11 | **Trade Analyzer** | V4 HCV-Hybrid 6-phase trade evaluation: deterministic SGP, stochastic MC (10K sims), Statcast signals, contextual adjustments, game theory, convergence diagnostics |
| 12 | **Trade Finder** | 5-tab: Smart Recs, Target Player, Browse Partners, Trade Readiness, Trade Values chart. Cosine partner matching + behavioral acceptance model |
| 14 | **Free Agents** | FA rankings by marginal value, ownership heat index, breakout candidate filter + waiver-priority tooling |
| 16 | **Player Compare** | Head-to-head comparison with category fit, schedule strength, SGP breakdown |
| 17 | **Leaders** | 7-tab: Leaders / Value / Breakouts / Prospects / Hot / Cold / Sell-High |
...
- **Trade Analyzer V4** (HCV-Hybrid, 6 phases) — Deterministic SGP with LP-constrained lineup totals, paired Monte Carlo with true antithetic variates + Gaussian copula, Statcast/Kalman/regime signal intelligence, Log5 matchup + injury process context, Bellman DP game theory, convergence diagnostics
...
- **~3,947 tests** across 165+ test files, ~17 skipped (PyMC/XGBoost/WeasyPrint optional deps)
