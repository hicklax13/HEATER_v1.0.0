...
def main():
    ...
    with main_col:
        ...
        # 2026-05-17 Section 5: Trade_Values page folded in as a 5th tab.
        tab_recs, tab_target, tab_browse, tab_readiness, tab_values = st.tabs(
            [
        ...
        with tab_values:
            _render_value_chart_tab(pool, config, all_team_totals, user_team_name)

    page_timer_footer("Trade Finder")


def _render_value_chart_tab(pool, config, all_team_totals, user_team_name) -> None:
    """Render the universal trade value chart (0-100 tier table).
    ...
    if has_context:
        try:
            contextual_df = compute_contextual_values(
                trade_values, user_totals, all_team_totals, user_team_name, config
            )
        except Exception:
            contextual_df = None
    ...
    with ctrl_col2:
        ...
        if has_context and contextual_df is not None:
            show_contextual = st.toggle("My Team Needs", value=False, key="tv_contextual_toggle")
    ...
    m1.metric("Total Players", f"{len(filtered)}")
    m2.metric("Elite Tier", f"{len(filtered[filtered[tier_col] == 'Elite'])}")
    m3.metric("Star Tier", f"{len(filtered[filtered[tier_col] == 'Star'])}")
    ...
    for tier_name in tier_order:
        ...
        display = pd.DataFrame(
            {
                "Rank": tier_players["rank"].values,
                "Player": tier_players[name_col].values,
                "Pos": tier_players["positions"].values,
                "Team": tier_players["team"].values,
                "Trade Value": tier_players[value_col].apply(lambda v: f"{int(round(v))}").values,
                "Dollar Value": tier_players["dollar_value"].apply(lambda v: f"${v:.2f}").values,
            }
        )
