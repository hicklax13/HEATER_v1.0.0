...
st.set_page_config(
    page_title="Heater | Lineup Optimizer", page_icon="", layout="wide", initial_sidebar_state="collapsed"
)
...
# ── Context panel: settings + opponent selector ───────────────────
...
with ctx:
    ...
    alpha = st.slider(
        ...
        value=0.5,
        step=0.05,
        help="0.0 = pure season-long optimization, 1.0 = pure Head-to-Head weekly focus, 0.5 = balanced.",
    )
    ...
    if opponent_names:
        selected_opponent = st.selectbox(
            ...
            index=0,
            help="Select your Head-to-Head opponent to enable matchup-specific optimization.",
        )
        ...
        if selected_opponent and opp_totals and my_totals:
            if H2H_AVAILABLE:
                try:
                    ...
                    render_context_card(
                        "Win Probability",
                        f'<div class="context-stat-row"><span class="context-stat-label">Probability</span>'
                        f'<span class="context-stat-value">{_win_prob:.1%}</span></div>'
                        f'<div class="context-stat-row"><span class="context-stat-label">Expected Wins</span>'
                        f'<span class="context-stat-value">{_exp_wins:.1f} / 10</span></div>'
                        f'<div class="context-stat-row"><span class="context-stat-label">Status</span>'
...
with main:
    ...
    tab_optimize, tab_h2h, tab_streaming, tab_analysis, tab_roster = st.tabs(
        ["Optimize", "Head-to-Head Matchup", "Streaming", "Category Analysis", "Roster"]
    )
    ...
    with tab_h2h:
        if not H2H_AVAILABLE:
            ...
            if not opponent_names:
                ...
                st.info(
                    "Select a Head-to-Head opponent in the settings panel to see per-category win probabilities "
    ...
    with tab_streaming:
        ...
        if streaming:
                ...
                stream_data.append(
                    {
                        ...
                        "Team": s.get("team", ""),
                        "Net Value (Standings Gained Points)": f"{s.get('net_value', 0):.3f}",
                        "Counting Standings Gained Points": f"{s.get('counting_sgp', 0):.3f}",
            ...
            st.caption(
                "Net Value = Counting stat Standings Gained Points gains minus ERA/WHIP rate damage. "
