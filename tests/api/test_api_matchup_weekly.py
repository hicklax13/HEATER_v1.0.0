...
def test_hitter_stats_weekly_scaled():
    ...
    assert _fmt_hitter_stats(row, weeks=7) == ["10/40", "8", "2", "8", "1", ".250", ".320"]
...
def test_pitcher_stats_weekly_scaled():
    ...
    assert _fmt_pitcher_stats(row, weeks=7) == ["10.0", "1", "0", "0", "12", "3.00", "1.10"]
...
def test_totals_weekly_scaled():
    ...
    assert out[0] == "10/40"  # 140/14 over 560/14
    assert out[1] == "8" and out[2] == "2"
    assert out[5] == ".250"  # AVG = 140/560 — ratio unchanged by scaling


def test_no_weeks_is_raw_backcompat():
    row = {"h": 70, "ab": 280, "r": 56, "hr": 14, "rbi": 56, "sb": 7, "avg": 0.250, "obp": 0.320}
    assert _fmt_hitter_stats(row) == ["70/280", "56", "14", "56", "7", ".250", ".320"]
