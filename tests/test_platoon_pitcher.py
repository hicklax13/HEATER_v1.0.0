...
class TestBayesianPlatoonAdjustment:
    ...
    def test_lhb_vs_rhp_no_individual_data_league_avg_boost(self):
        """LHB vs RHP with no individual data returns league-average boost (~1.075)."""
        result = bayesian_platoon_adjustment("L", "R")
        assert result == pytest.approx(1.075, abs=0.001)

    def test_rhb_vs_lhp_no_individual_data_smaller_boost(self):
        """RHB vs LHP with no data returns smaller boost (~1.058)."""
        result = bayesian_platoon_adjustment("R", "L")
        assert result == pytest.approx(1.058, abs=0.001)
    ...
    def test_same_side_lhb_vs_lhp_penalty(self):
        ...
        assert result == pytest.approx(1.0 - 0.075, abs=0.001)
        assert result < 1.0
    ...
    def test_same_side_rhb_vs_rhp_penalty(self):
        ...
        assert result == pytest.approx(1.0 - 0.058, abs=0.001)
        assert result < 1.0
    ...
    def test_200pa_individual_data_mostly_league_avg(self):
        ...
        # League advantage for LHB vs RHP = +7.5%
        # Blend weight = 200/1000 = 0.20
        # Blended = 0.20 * 0.0741 + 0.80 * 0.075 = 0.01481 + 0.060 = 0.07481
        ...
        expected = 1.0 + 0.20 * individual_adv + 0.80 * 0.075
        assert result == pytest.approx(expected, abs=0.002)
        # Should be mostly league average
        assert abs(result - 1.075) < abs(result - (1.0 + individual_adv))
    ...
    def test_1000pa_individual_data_50_50_blend(self):
        ...
        # Blended = 1.0 * 0.1111 + 0.0 * 0.075 = 0.1111
        result = bayesian_platoon_adjustment(
            "L",
            "R",
        ...
        expected = 1.0 + 1.0 * individual_adv + 0.0 * 0.075
        assert result == pytest.approx(expected, abs=0.002)
