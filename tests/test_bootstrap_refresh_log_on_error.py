...
def test_every_outermost_except_in_bootstrap_phase_records_refresh_log():
    """Outermost except in each ``_bootstrap_*`` function must record
    refresh_log on failure, or re-raise, or be an ImportError dep-check."""
    ...
    for node in ast.walk(tree):
        if not (isinstance(node, ast.FunctionDef) and node.name.startswith("_bootstrap_")):
            continue
        phase_count += 1
