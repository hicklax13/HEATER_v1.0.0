...
class TestBenchOptionValue:
    ...
    @patch("src.engine.output.trade_evaluator.get_free_agents", side_effect=lambda pool: pool)
    def test_one_for_two_trade_has_fa_pickup(self, mock_fa, sample_pool, config):
...
class TestLineupConstrainedEval:
    ...
    @patch("src.engine.output.trade_evaluator.get_free_agents", side_effect=lambda pool: pool)
    def test_fa_pickup_in_2for1_trade(self, mock_fa, sample_pool, config):
