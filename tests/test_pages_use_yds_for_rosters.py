...
def test_league_standings_uses_yds_for_rosters():
    """pages/6_League_Standings.py must NOT call load_league_rosters directly.
    Use yds.get_rosters() instead. load_league_records/schedule remain OK
    because YDS doesn't expose those."""
    bad = _scan_for_call(REPO_ROOT / "pages" / "6_League_Standings.py", "load_league_rosters")
    assert not bad, (
        f"BUG-009 regression: pages/6_League_Standings.py calls "
...
def test_player_compare_uses_yds_for_rosters():
    """pages/16_Player_Compare.py must NOT call load_league_rosters directly."""
    bad = _scan_for_call(REPO_ROOT / "pages" / "16_Player_Compare.py", "load_league_rosters")
    assert not bad, (
        f"BUG-009 regression: pages/16_Player_Compare.py calls load_league_rosters() directly. Offenders: {bad}"
    )
