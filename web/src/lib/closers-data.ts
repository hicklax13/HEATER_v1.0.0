 ...
 * no handcuffs, and no stats. To reach parity it should populate real
 * confidence levels + handcuffs + a numeric job-security % + proj SV/ERA/WHIP.
 */
...
/** Confidence label → 0–100 job-security score (drives the card's heat bar). */
...
// NOTE: every mlbId/teamId MUST pass `node web/scripts/audit-mock-ids.mjs`.
// The first four are reused from the live /api/closers response (verified).
